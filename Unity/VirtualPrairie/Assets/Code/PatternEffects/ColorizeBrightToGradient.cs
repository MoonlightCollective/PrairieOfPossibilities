...
public class ColorizeBrightToGradient : ColorizeBrightnessValue
{
	...
	public Color ColorForBrightness(float v, PrairieLayerScene scene )
	{
		float clampedV = Mathf.Clamp01(v);
		if (scene == null || scene.ScenePalette == null)
		{
		...
		{
			ColorPalette palette = scene.ScenePalette;
		...
		Color startLerpC = MinColor.Color(scene);
		Color endLerpC = MaxColor.Color(scene);
