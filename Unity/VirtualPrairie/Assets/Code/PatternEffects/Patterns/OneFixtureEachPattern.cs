using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Rendering.Universal;
using NaughtyAttributes;

public class OneFixtureEachPattern : PrairiePatternMonochromaticBase
{
	[Expandable]
	public OnePointEachSettings OnePointEachSettings;

	float _delayTimer = -1f;
	int _index = -1;

    public override void Run(float deltaTime, PrairieLayerGroup group, List<StemColorManager> points) 
    {
		_delayTimer -= deltaTime;
		if (_delayTimer <= 0 )
		{
            // move to the next fixture
    		_index++;
	        Debug.Log($"Lighting up Fixture {_index}");
			_delayTimer += OnePointEachSettings.DelayTimeSec;
			if (_index * PrairieDmxController.PointsPerFixture >= PrairieUtil.Points.Count)
			{
				_index = 0;
			}
		}
		Color blendColor = OnePointEachSettings.PointColor.Color(group) * BlendSettings.LayerAlpha * group.GroupAlpha;
        // light up every point in this fixture
        for (var pointIndex = _index * PrairieDmxController.PointsPerFixture; 
            pointIndex < (_index * PrairieDmxController.PointsPerFixture) + PrairieDmxController.PointsPerFixture; 
            ++pointIndex) 
        {
            points[pointIndex].SetColor(ColorBlend.BlendColors(blendColor,points[pointIndex].CurColor,BlendSettings.BlendMode));
        }
    }
}
