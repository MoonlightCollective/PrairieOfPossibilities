...
public static class RingTogglesFilterEffect extends LXEffect 
{
	...
	public RingTogglesFilterEffect(LX lx)
	{
		super(lx);
	...
	public void run(double deltaMs,double enabledAmount)
	{
		...
		for (int i = 0; i < PrairieUtils.kNumRings; i++)
		{
			...
			float newAlpha = rings[i].curValNormalized();
			for (LXModel m : model.sub(strFilter)) {
				for (LXPoint p : m.points) {
					int c = colors[p.index];
					float curAlpha = (LXColor.b(c) / 100.0f);
					float alpha = curAlpha * newAlpha;
					colors[p.index] = LXColor.rgba(LXColor.red(c),LXColor.green(c),LXColor.blue(c),(int)(alpha*255));
				}
			}
		}
	}
